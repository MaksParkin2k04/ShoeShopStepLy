...
namespace ShoeShop.Pages {
    public class IndexModel : PageModel {
        ...
        public async Task OnGetAsync(ProductSorting sort, int pageIndex = 1, Guid? categoryId = null) {
            Sorting = sort;
            ...
            if (categoryId.HasValue) {
                ...
                Products = allProducts.Where(p => p.CategoryId == categoryId.Value).Skip((pageIndex - 1) * 20).Take(20);
                TotalElementsCount = allProducts.Count(p => p.CategoryId == categoryId.Value);
            } else {
